...
export async function POST(
  ...
  try {
    ...
    const testimonialInfocms = await prismadb.testimonialInfo.create({
      data: {
        cmsPageId: cmsId,
        title: title,
        subtitle: subtitle,
    ...
    return NextResponse.json(testimonialInfocms);
  } catch (error) {
    console.log("[TESTIMONIAL_INFO_CMS_POST]", error);
...
export async function GET(
  ...
  try {
    ...
    const testimonialInfocms = await prismadb.testimonialInfo.findMany({
      where: {
        cmsPageId: cmsId,
      },
    });
